...
Next we will build the model using a backward step 
...
```{r}
  #check interactions in a stepwise forward fashion.
step(model, scope = . ~ .^2, direction = 'forward')

```

```{r}
model = glm(formula = outcome ~ grade + term + accOpen24 + home + income + 
    payment + bcOpen + delinq2yr + avgBal + rate + totalRevLim + 
    totalRevBal + payment:totalRevLim + term:avgBal + accOpen24:totalRevLim + 
    accOpen24:rate + grade:rate + term:home + payment:delinq2yr + 
    avgBal:rate + bcOpen:avgBal + delinq2yr:rate + income:avgBal + 
    totalRevLim:totalRevBal + accOpen24:payment + payment:totalRevBal + 
    accOpen24:delinq2yr + term:income + term:rate, family = "binomial", 
    data = train)
